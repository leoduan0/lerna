...
export function Navigation({ pages }: NavigationProps) {
  const { navigationStatus, setNavigationStatus } = useNavigationContext()

  return (
    <>
      <nav
        className={cn(
          "absolute z-20 flex h-full w-2/3 flex-col overflow-y-auto rounded-r-md border-border border-r bg-background/70 backdrop-blur-lg transition md:w-1/3 lg:static lg:w-96 lg:translate-x-0 lg:rounded-none lg:backdrop-blur-none",
          navigationStatus
            ? "translate-x-0 opacity-100"
            : "-translate-x-full opacity-0 lg:opacity-100",
        )}
      >
        <div className="flex h-12 w-full items-center justify-center space-x-2 border-border border-b">
          <svg
      ...
      <Button
        className={cn(
          "absolute right-0 left-2/3 h-full rounded-none bg-black p-0 transition md:left-1/3",
          navigationStatus
